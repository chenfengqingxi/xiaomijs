...
// 底下的双下标轮播结束
    ...
    //    banner左侧
        ...
        let  ce=document.querySelectorAll('ul.celan li')
        let displ=document.querySelectorAll('.bannertop li')
        let celan=document.querySelector('ul.bannertop')
        let dis=document.querySelector('ul.celan')
         ce.forEach(function (dom,index) {
             dom.onmouseenter=function () {
                displ.forEach(function (a,b) {
                    a.classList.remove('z')
                })
                 displ[index].setAttribute('class','z')
             }
            dis.onmouseleave=function () {
                 // ce.onmouseleave=function () {
                     displ.forEach(function (val) {
                         val.classList.remove('z')
                     })
                 // }
             }
         })
    ...
    //    导航
let daohang=document.querySelectorAll('.daohang-wenzi a')
let di=document.querySelectorAll('.daohangdibu li')
   daohang.forEach(function (dom,index) {
       dom.onmouseenter=function () {
           di.forEach(function (a,b) {
             a.classList.remove('s')
           })
           animate(di[index],{top:100},200)
           di[index].setAttribute('class','s')
       }
       dom.onmouseleave=function () {
           di.forEach(function (val) {
               val.classList.remove('s')
           })
       }
   })
